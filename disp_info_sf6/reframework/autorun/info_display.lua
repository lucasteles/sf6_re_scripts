local gBattle
local changed
local display_player_info = false
local display_projectile_info = false

function create_player_config()
    return {
        absolute_range = 0,
        relative_range = 0
    }
...
local p1 = create_player_config()
local p2 = create_player_config()
        ...
        if display_player_info then
            ...
            if imgui.tree_node("P1") then
                ...
                if imgui.tree_node("State Info") then
                    ...
                    imgui.multi_color("Action Frame:",
                        math.floor(read_sfix(p1.mActionFrame)) .. " / " .. math.floor(read_sfix(p1.mMarginFrame)) ..
                            " (" .. math.floor(read_sfix(p1.mEndFrame)) .. ")")
            ...
            if imgui.tree_node("P2") then
                ...
                if imgui.tree_node("State Info") then
                    ...
                    imgui.multi_color("Action Frame:",
                        math.floor(read_sfix(p2.mActionFrame)) .. " / " .. math.floor(read_sfix(p2.mMarginFrame)) ..
                            " (" .. math.floor(read_sfix(p2.mEndFrame)) .. ")")
